...
if st.button("Submit"):
    ...
    if response.status_code == 200:
        ...
        relevant_response = generated_text.split("Response:")[-1].strip().replace("<|end_of_text|>", "")
        relevant_response = re.sub(r'\b\d\b', '', relevant_response).strip()
        st.write(relevant_response)
        
        # Extract keywords from the generated text
        keywords = extract_keywords(relevant_response)
        print(keywords)
